...
$patch = Join-Path $repoRoot 'tools\patch_fujinet_fastiwm_p02c5_buslock.py'
if (-not (Test-Path -LiteralPath $patch -PathType Leaf)) {
    throw ('P0.2C5 firmware patch missing: ' + $patch)
}
...
Write-Host ''
Write-Host '=== FujiNet P0.2C5 explicit SPI bus-acquire build ===' -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) {
    throw ('P0.2C5 firmware patch failed with exit code ' + $LASTEXITCODE)
}
...
foreach ($symbol in @(
    ...
    if (@($nmLines | Where-Object { $_ -match $symbol }).Count -eq 0) {
        throw ('Required P0.2C5 symbol missing from firmware ELF: ' + $symbol)
    }
}
Write-Host 'P0.2C5 ELF symbol verification PASS.' -ForegroundColor Green

$release = Join-Path $repoRoot 'build\fastiwm-p0.2c5-firmware'
if (Test-Path -LiteralPath $release) {
    Remove-Item -LiteralPath $release -Recurse -Force
}
New-Item -ItemType Directory -Path $release -Force | Out-Null
Copy-Item -LiteralPath $firmwareBin `
    -Destination (Join-Path $release 'fujinet-p0.2c5-firmware.bin') -Force
Copy-Item -LiteralPath $firmwareElf `
    -Destination (Join-Path $release 'fujinet-p0.2c5-firmware.elf') -Force
Copy-Item -LiteralPath $platformIni `
    -Destination (Join-Path $release 'platformio.production-rev1.ini') -Force

@'
FujiNet P0.2C5 - explicit Fast-IWM SPI bus acquisition

Keep using FASTPROBE-P0.2C.po.

P0.2C4 proved the exact arm block is decoded and the delayed autosend
reaches FASTIWM C4 AUTO TX START, but the private SPI transmit never
returned.  P0.2C5 explicitly acquires the spifast device bus while
interrupts and the scheduler are still enabled, then disables interrupts
only around the physical polling transaction.  The bus is released after
transmit.
...
If ACQUIRE START appears without ACQUIRE DONE, another transaction still
owns the bus.  If ACQUIRE DONE appears but BUS RELEASE does not, the hang
is inside spi_device_polling_transmit itself.
...
'@ | Set-Content -LiteralPath (Join-Path $release 'README-P0.2C5.txt') -Encoding ASCII
...
Write-Host ''
Write-Host 'FUJINET P0.2C5 BUILD COMPLETE' -ForegroundColor Green
Write-Host ('Firmware: ' + (Join-Path $release 'fujinet-p0.2c5-firmware.bin'))
