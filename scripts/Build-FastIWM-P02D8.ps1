...
$hostOverlay = Join-Path $repoRoot `
    'tools\patch_spbench_fastiwm_p02d8_control_retry.py'
$firmwareOverlay = Join-Path $repoRoot `
    'tools\patch_fujinet_fastiwm_p02d8_reconnect_diag.py'
...
if (-not $PackageExisting) {
    & powershell.exe -NoProfile -ExecutionPolicy Bypass `
        ...
        -Experiment 'P0.2D8' `
        -FirmwareExperiment 'P0.2D8' `
        -OverlayPatch $hostOverlay
    if ($LASTEXITCODE -ne 0) {
        throw ('P0.2D8 host build failed with exit code ' + $LASTEXITCODE)
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw ('P0.2D8 firmware build failed with exit code ' + $LASTEXITCODE)
    }
}
...
& powershell.exe -NoProfile -ExecutionPolicy Bypass `
    ...
    -PackageExisting
if ($LASTEXITCODE -ne 0) {
    throw ('P0.2D8 paired packaging failed with exit code ' + $LASTEXITCODE)
}
