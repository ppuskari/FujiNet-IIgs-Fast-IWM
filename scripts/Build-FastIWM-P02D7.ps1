...
$hostOverlay = Join-Path $repoRoot `
    'tools\patch_spbench_fastiwm_p02d7_guarded_exit.py'
$firmwareOverlay = Join-Path $repoRoot `
    'tools\patch_fujinet_fastiwm_p02d7_isr_guard.py'
...
if (-not $PackageExisting) {
    & powershell.exe -NoProfile -ExecutionPolicy Bypass `
        ...
        -Experiment 'P0.2D7' `
        -FirmwareExperiment 'P0.2D7' `
        -OverlayPatch $hostOverlay
    if ($LASTEXITCODE -ne 0) {
        throw ('P0.2D7 host build failed with exit code ' + $LASTEXITCODE)
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw ('P0.2D7 firmware build failed with exit code ' + $LASTEXITCODE)
    }
}
...
& powershell.exe -NoProfile -ExecutionPolicy Bypass `
    ...
    -PackageExisting
if ($LASTEXITCODE -ne 0) {
    throw ('P0.2D7 paired packaging failed with exit code ' + $LASTEXITCODE)
}
