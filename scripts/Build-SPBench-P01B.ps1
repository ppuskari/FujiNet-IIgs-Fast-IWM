...
$buildRoot = Join-Path $repoRoot 'build\spbench-p0.1b'
...
$imagePath = Join-Path $releaseRoot 'SPBENCH-P0.1B.po'
$catalogPath = Join-Path $releaseRoot 'SPBENCH-P0.1B.catalog.txt'
...
$finalZip = Join-Path $buildRoot 'SPBENCH-P0.1B.zip'
...
Write-Host 'SPBENCH P0.1B build'
...
@(
    ...
    '',
    'GS/OS is used only to identify the prefix block device.',
    'The timed loop calls the slot SmartPort firmware directly.',
    'Mode: extended READBLOCK command $41, 512 bytes per call.',
    '',
    'Sequence:',
    ...
    '',
    'Record the complete screen output for stock 2.8 MHz first.',
    '',
    'Compare directly with P0.1A GS/OS DRead results:',
    '  1 MiB: 16530 B/s, 132 kbit/s',
    '  4 MiB: 16421 B/s, 131 kbit/s'
