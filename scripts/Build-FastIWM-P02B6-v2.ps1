...
The host makes one standard SmartPort STATUS $AA call to unit $01.
FujiNet returns its ordinary HELLO WORLD status response at 4-us timing,
then schedules one private 2-MHz packet 20 ms later.

After the ROM call returns, FASTPROBE verifies idle IWM mode and waits in
the direct IWM read loop. No private 1110/1111 phase signature is used.

Production Rev1 runtime detection is unchanged.
