...
$hostOverlay = Join-Path $repoRoot 'tools\patch_spbench_fastiwm_p02d0.py'
$firmwareOverlay = Join-Path $repoRoot 'tools\patch_fujinet_fastiwm_p02d0_pcm.py'
...
if ($LASTEXITCODE -ne 0) {
    throw ('P0.2D0 matched build failed with exit code ' + $LASTEXITCODE)
}
