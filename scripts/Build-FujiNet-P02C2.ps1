...
$patch = Join-Path $repoRoot 'tools\patch_fujinet_fastiwm_p02c2.py'
if (-not (Test-Path -LiteralPath $patch -PathType Leaf)) {
    throw ('P0.2C2 firmware patch missing: ' + $patch)
}
...
Write-Host ''
Write-Host '=== FujiNet P0.2C2 one-reset arm-hold build ===' -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) {
    throw ('P0.2C2 firmware patch failed with exit code ' + $LASTEXITCODE)
}
...
foreach ($symbol in @(
    ...
    if (@($nmLines | Where-Object { $_ -match $symbol }).Count -eq 0) {
        throw ('Required P0.2C2 symbol missing from firmware ELF: ' + $symbol)
    }
}
Write-Host 'P0.2C2 ELF symbol verification PASS.' -ForegroundColor Green

$release = Join-Path $repoRoot 'build\fastiwm-p0.2c2-firmware'
if (Test-Path -LiteralPath $release) {
    Remove-Item -LiteralPath $release -Recurse -Force
}
New-Item -ItemType Directory -Path $release -Force | Out-Null
Copy-Item -LiteralPath $firmwareBin `
    -Destination (Join-Path $release 'fujinet-p0.2c2-firmware.bin') -Force
Copy-Item -LiteralPath $firmwareElf `
    -Destination (Join-Path $release 'fujinet-p0.2c2-firmware.elf') -Force
Copy-Item -LiteralPath $platformIni `
    -Destination (Join-Path $release 'platformio.production-rev1.ini') -Force

@'
FujiNet P0.2C2 - one-reset arm hold

Keep using FASTPROBE-P0.2C.po.

P0.2C correctly arms on standard READBLOCK $7FA55A, but its generic
SmartPort reset handler cleared fast_iwm_probe_armed when the arm
transaction itself terminated. P0.2C2 gives that freshly armed state a
one-reset grace token. The next 1011 consumes the arm and queues the
private 2-MHz packet. A later unexpected reset still cancels stale state.
...
'@ | Set-Content -LiteralPath (Join-Path $release 'README-P0.2C2.txt') -Encoding ASCII
...
Write-Host ''
Write-Host 'FUJINET P0.2C2 BUILD COMPLETE' -ForegroundColor Green
Write-Host ('Firmware: ' + (Join-Path $release 'fujinet-p0.2c2-firmware.bin'))
