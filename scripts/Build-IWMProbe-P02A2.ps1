...
$patchScript = Join-Path $repoRoot 'tools\patch_iwmprobe_p02a2.py'
...
Write-Host 'IWMPROBE P0.2A2 build'
...
if ($patchExit -ne 0) {
    throw ('P0.2A2 patch failed with exit code ' + $patchExit)
}
...
$newImage = Join-Path $releaseRoot 'IWMPROBE-P0.2A2.po'
...
Write-Host ''
Write-Host 'IWMPROBE P0.2A2 BUILD COMPLETE' -ForegroundColor Green
